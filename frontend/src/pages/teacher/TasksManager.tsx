...
interface Task {
    ...
    title: string;
    description: string;
    is_visible: boolean;  // ← ДОБАВИТЬ ПОЛЕ
    created_at: string;
}
...
export default function TasksManager({ onNavigate }: TasksManagerProps) {
    ...
    // ← НОВАЯ ФУНКЦИЯ: переключение видимости
    const toggleVisibility = async (task: Task) => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`http://localhost:8000/api/tasks/${task.id}/toggle-visibility`, {
                method: 'PATCH',
                headers: {
                    'Authorization': token ? `Bearer ${token}` : ''
                }
            ...
            setMessage({ type: 'error', text: 'Ошибка при изменении видимости' });
            setTimeout(() => setMessage(null), 3000);
        }
    };
        ...
        <Layout title="Управление задачами" role="teacher" onLogout={handleLogout} onNavigate={onNavigate}>
            ...
            {loading ? (
                ...
                <div className="tasks-list">
                    {tasks.map((task) => (
                        <div key={task.id} className={`task-item ${!task.is_visible ? 'task-hidden' : ''}`}>
                            <div className="task-content">
                                <div className="task-title">
                                    {task.title}
                                    {!task.is_visible && (
                                        <span className="hidden-badge">Скрыта</span>
                                    )}
                                </div>
                                {task.description && <div className="task-desc">{task.description}</div>}
                                <div className="task-date">{new Date(task.created_at).toLocaleDateString()}</div>
                            </div>
                            <div className="task-actions task-actions-icon-only">
    <button 
        onClick={() => toggleVisibility(task)} 
        className={`visibility-btn ${task.is_visible ? 'visible' : 'hidden'}`}
        title={task.is_visible ? 'Скрыть от учеников' : 'Показать ученикам'}
    >
        {task.is_visible ? 'скрыть' : 'показать'}
    </button>
    <button onClick={() => handleEdit(task)} className="edit-btn" title="Редактировать">
        Редактировать
    </button>
    <button onClick={() => handleDelete(task.id)} className="delete-btn" title="Удалить">
        Удалить
    </button>
</div>
                        </div>
